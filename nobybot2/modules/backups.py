...
from nobybot2 import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER
from nobybot2.__main__ import DATA_IMPORT
...
def export_data(update, context):
    ...
    f = open("Sung Jin-Woo {}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Sung Jin-Woo {}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Sung Jin-Woo -Backup` was specially made for notes."
        .format(chat.title, chat_id, tgl),
        timeout=360,
...
*Only for group owner:*
 ...
 ✪︎ /import: Reply to the backup file for the butler / emilia group to import as much as possible, making transfers very easy! \
 Note that files / photos cannot be imported due to telegram restrictions.

 ✪︎ /export: Export group data, which will be exported are: rules, notes (documents, images, music, video, audio, voice, text, text buttons) \
